...
var updateStatus = function() {
    dataFocus.nodes.forEach(function(e, val) {
    ...
    dataResearch.nodes.forEach(function(e, val) {
        researchStat[e.id] = researchStat[e.id] ? researchStat[e.id] : 0;
        if (researchStat[e.id] != 2) {
            let complete = true;
            e.require.forEach(function(e1) {
                if (researchStat[e1] != 2) {
                    complete = false;
                }
            });
            if (complete === true) {
...
var tooltipChange = function(e) {
    ...
    if (focusStat[e.id] == 2) {
        ...
        if (e.require.length > 0) {
            e.require.forEach(function(e1) {
                text += '需要<b class="f-green">' + dataFocus.nodes[e1].label + '</b><br/>';
            });
        }
    } else {
        ...
        if (e.require.length > 0) {
            e.require.forEach(function(e1) {
                text += '需要<b class="f-red">' + dataFocus.nodes[e1].label + '</b><br/>';
            });
        }
    }
    text += '<br/><b class="f-orange">效果：</b><br/>' + getPolicyEffects(e.effect?e.effect:[]);
    return text;
};
...
dataFocus.nodes.forEach(function(node) {
    g.setNode(node.id + '', {
        width: 150,
        height: 50
    });
...
dataFocus.edges.forEach(function(edge) {
...
g.nodes().forEach(function(node, i) {
    coord = g.node(node);
    dataFocus.nodes[i].x = coord.x;
    dataFocus.nodes[i].y = coord.y;
});
g.edges().forEach(function(edge, i) {
    coord = g.edge(edge);
    var startPoint = coord.points[0];
    var endPoint = coord.points[coord.points.length - 1];
    dataFocus.edges[i].startPoint = startPoint;
    dataFocus.edges[i].endPoint = endPoint;
    dataFocus.edges[i].controlPoints = coord.points.slice(1, coord.points.length - 1);
});
        ...
        if (focusStat[node.id] == 2) {
            ...
            if (node.require.length > 0) {
                node.require.forEach(function(e1) {
                    req += '需要<b class="f-green">' + dataFocus.nodes[e1].label + '</b><br/>';
                });
            }
